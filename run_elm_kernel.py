...
from htmresearch.algorithms.KernelOS_ELM import KOSELM

#plt.ion()

def initializeELMnet(nDimInput, nDimOutput, numNeurons=10,BN=True, forgettingFactor = 0.99,PRINTING=True):
  ...
  net = OSELM(nDimInput, nDimOutput,
            numHiddenNeurons=numNeurons, activationFunction='sig',BN=BN, forgettingFactor=forgettingFactor)
  ...
  if PRINTING:
    ...
    print('input weight forgetting factor = '+str(1))
    print('output weight forgetting factor = ' + str(forgettingFactor))
    print('---------------------------------------------------')
...
if __name__ == "__main__":
  ...
  net = initializeELMnet(nDimInput=X.shape[1],
                         nDimOutput=1, numNeurons=1600,BN=False,forgettingFactor =0.9)
