...
from htmresearch.algorithms.AOS_ELM import AOSELM

#plt.ion()

def initializeAOSELMnet(nDimInput, nDimOutput, numNeurons=10,BN=True,OutWeightFF=0.999,InWeightFF=0.999,AE=True,VFF_RLS=False):
...
if __name__ == "__main__":
  ...
  print 'mean = ',meanSeq
  print 'standard deviation = ',stdSeq
  ...
  random.seed(6)
  net = initializeAOSELMnet(nDimInput=X.shape[1],
                            nDimOutput=1,
                            numNeurons=400,
                            BN=True,
                            OutWeightFF=1,
  ...
  for i in xrange(nTrain, len(sequence)-predictionStep-1):
    ...
    print net.forgettingFactor

  predictedInput = (predictedInput * stdSeq) + meanSeq
  targetInput = (targetInput * stdSeq) + meanSeq
  trueData = (trueData * stdSeq) + meanSeq

  saveResultToFile(dataSet, predictedInput, 'Decay'+str(net.forgettingFactor)+'WAOSELM'+str(net.numHiddenNeurons))
