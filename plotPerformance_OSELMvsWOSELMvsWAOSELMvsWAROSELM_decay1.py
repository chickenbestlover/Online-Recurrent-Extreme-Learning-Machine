...
def load_plot_return_errors(decay,algorithmname,nHidden,dataset='nyc_taxi',skipTrain=500,window=100):
    decay = str(decay)
    nHidden = str(nHidden)
    filepath = './prediction/' + dataSet + '_Decay'+decay+algorithmname+nHidden+'_pred.csv'
...
if __name__ == "__main__":
  ...
  plt.legend(loc='lower right')
  plt.legend()
  plt.savefig(figPath + 'squrare_deviation_Decay'+str(decay)+'OSELMvsWOSELMvsWAOSELMvsWAROSELM.pdf')
  ...
  plt.savefig(figPath + 'model_performance_summary_Decay'+str(decay)+'OSELMvsWOSELMvsWAOSELMvsWAROSELM.pdf')
