...
from algorithms.AOS_ELM import AOSELM
...
def initializeNet(nDimInput, nDimOutput, numNeurons=100, algorithm='ORELM',
  ...
  if algorithm=='FOSELM':
      '''
      Fully Online Sequential ELM (FOSELM). It's just like the basic OSELM, except initialization.
      '''
  ...
  if algorithm=='NFOSELM':
      ...
      '''
      FOSELM + layer Normalization + input layer weight Auto-encoding.
      '''
      net = AOSELM(nDimInput, nDimOutput,
                  numHiddenNeurons=numNeurons,
                  activationFunction='sig',
                  LN=LN,
      ...
      FOSELM + layer normalization + input layer weight auto-encoding + hidden layer weight auto-encoding.
      '''
...
def _getArgs():
  ...
  parser.add_option("-a",
                    ...
                    type=str,
                    default='AOSELM',
                    help="Algorithm name, choose from ORELM, FOSELM, AOSELM")
...
if __name__ == "__main__":
  ...
  plt.title('Time-series Prediction of '+algorithm+' on '+dataSet+' dataset' )
  plt.draw()
  plt.show()
