...
class REOSELM(object):
  ...
  def initializePhase(self, features, targets,lamb=0.001):
    """
    ...
    Ht = np.transpose(Ht)
    HtH = np.dot(Ht,H0)
    self.M = inv(HtH)+lamb*np.eye(HtH.shape[1])
    self.beta = np.dot(np.dot(self.M,Ht), targets)
  ...
  def train(self, features, targets):
    ...
    try:
      self.M -= np.dot(self.M,
                       np.dot(Ht, np.dot(
          inv(np.eye(numSamples) + np.dot(H, np.dot(self.M, Ht))),
          np.dot(H, self.M))))
